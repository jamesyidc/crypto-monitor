...
#### 2. 最大连续K线数 (max_streak)
- **定义**：该币种历史上连续处于"上涨占优"状态的最大K线根数
- **类型**：INTEGER
- **计算方式**：
  - 遍历该币种的所有历史K线（按时间顺序）
...
- **定义**：最大连续上涨占优周期的第一根K线的时间
- **类型**：TEXT (ISO 8601格式：YYYY-MM-DDTHH:mm:ss.sssZ)
- **计算方式**：当连续计数达到新的最大值时，记录该连续周期的起始K线时间
- **说明**：标识强势周期的起点，用于回溯分析

#### 4. 结束时间 (max_streak_end_time)
- **定义**：最大连续上涨占优周期的最后一根K线的时间
- **类型**：TEXT (ISO 8601格式：YYYY-MM-DDTHH:mm:ss.sssZ)
- **计算方式**：当连续计数达到新的最大值时，记录当前K线时间
- **说明**：标识强势周期的终点，用于计算周期持续时长

#### 5. 当前连续 (current_streak)
- **定义**：截止到最新K线，当前正在进行的连续上涨占优K线根数
- **类型**：INTEGER
- **计算方式**：
  - 如果最新K线满足"上涨占优"，则 current_streak = 上一次的 current_streak + 1
  - 如果最新K线不满足"上涨占优"，则 current_streak = 0（重置）
- **说明**：
...
1. **禁止自行计算占比**：必须使用 `klineService.getKlineWithIndicators()` 已计算好的字段
2. **数据表字段命名**：`last_high_ratio` 和 `last_low_ratio` 字段名有误导性，实际存储的是"占比上涨"和"占比下跌"
3. **时间格式**：所有时间字段必须使用 ISO 8601 格式
4. **连续计数规则**：一旦不满足"上涨占优"条件，连续计数立即重置为0
