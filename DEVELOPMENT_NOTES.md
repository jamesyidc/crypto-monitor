...
> **最后更新**: 2025-10-29 16:00
> **系统状态**: ✅ 稳定运行
> **公网访问**: https://3000-ij3odq6k2fvoix4jt5np8-2e77fc33.sandbox.novita.ai
> **最新操作**: ✅ 修复震荡收敛带宽均值统计逻辑 + 调整精度和数据保留策略
> **数据状态**: ✅ 完整（币种29个，K线数据保留60天）

---

## 🔧 最新修改记录（2025-10-29 16:00）

### 1. 震荡收敛带宽均值 - 精度调整
- **修改内容**: 平均值从保留2位小数改为4位小数
- **文件位置**: `/home/user/webapp/public/static/kline.js` 第728行
- **修改前**: `.toFixed(2)` → 显示 `3.10`
- **修改后**: `.toFixed(4)` → 显示 `3.1042`
- **原因**: 用户要求更高精度，便于精确分析

### 2. K线数据保留策略调整
- **修改内容**: 从保留30天改为保留60天
- **文件位置**: `/home/user/webapp/src/services/klineService.ts` 第134行
- **修改前**: `cleanOldKlineDataByDays(config.symbol, timeframe, 30)`
- **修改后**: `cleanOldKlineDataByDays(config.symbol, timeframe, 60)`
- **数据量**: 60天 ≈ 17,280条5分钟K线/币种
- **存储空间**: 29个币种 × 17,280条 ≈ 50万条记录
- **原因**: 保留更多历史数据，便于长期趋势分析

### 3. 数据库备份策略
- **自动备份**: 每12小时一次
- **保留数量**: 最近3次备份
- **备份位置**: `/home/user/webapp_db_backup_*.tar.gz`
- **预估大小**: 约10-15MB/备份（60天数据）
...
### 功能名称
**震荡收敛带宽均值** - 在K线页面右上角统计窗口显示

### 功能目的
计算最近一次起涨前的震荡收敛阶段中，布林带带宽最小的3个值的平均数，用于识别收敛后突破的力度参考。

### 核心逻辑（用户明确要求）

#### 步骤1：找到离现在最近的一个起涨点
- **起涨点定义**：累计20根K线涨跌幅 > +2%
- **计算方法**：从当前K线往前数20根，计算这20根的change字段累加值
- **重要**：只找**最近的1个**起涨点（数组索引最小的，即最新的）

#### 步骤2：从起涨点向前查找震荡收敛状态
- **震荡收敛定义**：`channel_state` 字段包含 "震荡收敛" 文字
- **查找方向**：从起涨点的前一根K线（index-1）开始，向数组后方（更早的K线）查找
- **停止条件**：找到第一个"震荡收敛"状态就停止（距离起涨点最近的）

#### 步骤3：收集区间内的带宽值
- **区间定义**：从【震荡收敛K线】到【起涨点K线】之间的所有K线
- **带宽计算**：`bandwidth = boll_ub - boll_lb`（注意：不使用 `boll_width` 字段，该字段为null）
- **数据方向**：前端数组索引0是最新数据，索引越大越早
...
klineData = [
  {
    time: "2025/10/29 15:35:00",  // 最新数据，索引0
    change: "-0.09%",
    boll_ub: 562.2654,
    boll_lb: 560.3346,
    channel_state: "放量突破 ⚡",
  ...
  {
    time: "2025/10/29 15:30:00",  // 索引1
    change: "-0.12%",
    boll_ub: 562.2191,
    boll_lb: 560.4309,
    channel_state: "下跌衰竭 ⚠️",
    // ...
  },
  // ... 更早的数据
]
...
// 输出结果示例
{
  avgConvergenceBandwidth: "3.10",  // 平均值，2位小数
  convergenceFound: true,           // 是否找到震荡收敛
  sampleCount: 3,                   // 样本数量
  risingPointIndex: 15,             // 起涨点索引
  convergenceIndex: 25              // 震荡收敛索引
}
```
...
```javascript
function calculateConvergenceBandwidth(todayData) {
  // 1. 找到最近的一个起涨点（索引最小的）
  let risingPointIndex = null;
  for (let i = 0; i < todayData.length - 20; i++) {
    let cumulative = 0;
    for (let j = i; j < i + 20; j++) {
      cumulative += parseFloat(todayData[j].change);
    }
    if (cumulative > 2) {
      risingPointIndex = i;  // 找到第一个就停止
      break;
    }
  }
  ...
  for (let i = risingPointIndex + 1; i < todayData.length; i++) {
    if (todayData[i].channel_state.includes('震荡收敛')) {
      convergenceIndex = i;  // 找到第一个就停止
      break;
    }
  }
  
  if (convergenceIndex === null) {
    return null;  // 没有找到震荡收敛
  }
  
  // 3. 收集区间内的带宽值
  const bandwidths = [];
  for (let i = risingPointIndex; i <= convergenceIndex; i++) {
    const k = todayData[i];
    if (k.boll_ub && k.boll_lb) {
      const bandwidth = k.boll_ub - k.boll_lb;
  ...
  const top3 = bandwidths.slice(0, 3);
  const avg = (top3[0] + top3[1] + top3[2]) / 3;
...
- 起涨点：15:20，累计涨幅 +2.5%
- 震荡收敛：14:30
- 区间带宽值：[3.2444, 2.9946, 3.0736, 3.5123, ...]
- 最小3个：3.2444, 2.9946, 3.0736
- 平均值：(3.2444 + 2.9946 + 3.0736) / 3 = 3.1042
- 显示：`3.10`
...
### 注意事项
1. **数据方向**：前端数组索引0是最新数据，索引越大数据越早
2. **只找最近1个**：不要遍历所有起涨点，只找最近的1个
3. **带宽计算**：使用 `boll_ub - boll_lb`，不要用 `boll_width` 字段
4. **区间包含边界**：区间包含起涨点和震荡收敛点本身
5. **至少3个样本**：如果区间内带宽值少于3个，显示"无数据"
6. **精度要求**：平均值保留小数点后4位
...
### 功能名称
**高抛提示** - 在K线表格的"信号"列显示橙色"高抛"标签

### 功能目的
在多头信号持续但动能减弱时，提示用户考虑高位减仓，避免追高风险。
...
2. **SAR变化%在增加**
   - 当前K线的 `sarChangePercent` > 上一根K线的 `sarChangePercent`
   - 说明SAR与价格距离在拉大，多头动能可能减弱
...
```html
<span class="inline-block px-2 py-1 bg-orange-500 text-white text-xs rounded font-bold" 
      title="满足高抛条件：多头信号+SAR变化%增加+涨跌幅<0.1%+RSI5分钟>69">
  高抛
</span>
```

- **位置**：独立的"操作提示"列（第3列）
- **列头**：橙色背景高亮（`bg-orange-50`）
- **单元格背景**：橙色背景（`bg-orange-50`），起涨/起跌点时继承行背景色
- **标签颜色**：橙色背景（`bg-orange-500`）+ 白色文字
- **提示**：鼠标悬停显示触发条件
- **空值显示**：不满足条件时显示"-"
...
- **列头背景**：蓝色（`bg-blue-50`）
- **显示内容**：累计20根K线涨跌幅标记
  - 起涨点（>2%）：绿色标签（`bg-green-600`）
  - 起跌点（<-3%）：红色标签（`bg-red-600`）
  - 普通状态：灰色标签（`bg-gray-300`）
- **行背景联动**：与时间列保持一致的背景色

### 实现位置
- **HTML文件**：`/home/user/webapp/public/kline.html`
  - 起涨/起跌点列头：第186行
  - 操作提示列头：第187行
- **JS文件**：`/home/user/webapp/public/static/kline.js`
  - 高抛判断逻辑：第417-444行
  - 起涨/起跌点计算：第364-387行
  - 起涨/起跌点标记生成：第463-470行
  - 表格渲染：第484-492行
...
### 清理机制
- **触发时机**：每次同步K线数据后自动执行
- **清理逻辑**：删除 `open_time < (当前时间 - 60天)` 的记录
- **实现位置**：`/home/user/webapp/src/services/klineService.ts` 第134行
...
### 计算说明
- **5分钟K线**：1天 = 288根，60天 = 17,280根
- **存储空间**：约占用 10-15MB（29个币种 × 17,280根 × 约20字节/根）
- **查询性能**：60天数据量适中，不影响查询速度

### 注意事项
1. **自动清理**：每次K线同步后自动执行，无需手动干预
2. **数据安全**：只删除60天以前的数据，不影响近期数据
3. **备份策略**：建议定期备份数据库（已有12小时自动备份）
