...
### 需求描述
根据**当前时间段**和**风险提示次数**，动态计算并显示风险等级。不同时间段有不同的阈值标准。

### 时间段风险阈值规则

#### 1. 凌晨时段（0:00-6:00）
- **低风险**：风险提示次数 < 3
- **中风险**：风险提示次数 3-4（含3，不含4）
- **高风险**：风险提示次数 ≥ 4

#### 2. 上午时段（6:00-12:00）
- **低风险**：风险提示次数 < 4
- **中风险**：风险提示次数 4-5（含4，不含5）
- **高风险**：风险提示次数 ≥ 5

#### 3. 下午时段（12:00-18:00）
- **低风险**：风险提示次数 < 5
- **中风险**：风险提示次数 5-6（含5，不含6）
- **高风险**：风险提示次数 ≥ 6

#### 4. 晚间时段（18:00-24:00）
- **低风险**：风险提示次数 < 6
- **中风险**：风险提示次数 6-7（含6，不含7）
- **高风险**：风险提示次数 ≥ 7

### 实现逻辑

**文件位置：** `/home/user/webapp/public/static/app.js`
...
- ✅ 首页风险提示卡片动态显示风险等级和时间段
- ✅ 颜色编码自动根据风险等级变化

### v1.8 - 2025-10-28 22:00 & 22:20
- ✅ 新增K线起涨起跌结构识别功能
